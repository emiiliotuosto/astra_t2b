...
- GSSI:
  ...
  - Inverso, Tuosto, and Sales (a PhD student at GSSI) have applied the sequentialisation technique to precisely detect races in multi-threaded C programs; a paper describing the methodology and the related tool is under review. The approach instruments C-like code in order to establish if shared memory can possibly accessed by concurrent operation at least one of which is a write operation. The approach leverages C-Seq for the analysis. Preliminary results indicate that, at the cost of a slightly less efficient analysis, the method rules out false positives and negatives within the given bounds.
